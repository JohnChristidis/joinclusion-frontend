...
export function TopicModal({ isOpenTopic, onCloseTopic, handleCharacterSelect, mod, handleMod, handleTopicSelect}) {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10 z-50">
      <div className="bg-white rounded-lg p-4 w-100 md:h-128 xl:h-160 max-w-lg">
        ...
        <div className="max-h-96 md:max-h-128 xl:max-h-160 overflow-y-auto"> {/* Added max height and overflow */}
          <TopicAccordion mod={mod} handleMod={handleMod} handleTopicSelect={handleTopicSelect}/>
        </div>
        <div className="flex justify-end mt-4">
          <button className="bg-blue-500 text-white px-4 py-2 rounded" onClick={onCloseTopic}>
            Close
          </button>
        </div>
      </div>
